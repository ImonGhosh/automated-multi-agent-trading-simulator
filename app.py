...
class Trader:
    ...
    def get_logs(self, previous=None) -> str:
        ...
        if response != previous:
            return response
        return gr.update() # Gradio leaves the component as-is
...
class TraderView:
    ...
    def make_ui(self):
        ...
        timer = gr.Timer(value=120) # this event fires every 120 seconds
        timer.tick( 
            fn=self.refresh, # attaches this tick event to the self.refresh function
            inputs=[], # takes no inputs
            outputs=[ # retrieves updated values for these components for live UI feed
        ...
        log_timer = gr.Timer(value=0.5) # this event fires every 0.5 seconds (much more frequent)
        log_timer.tick(
            fn=self.trader.get_logs, # attaches this tick event to the self.trader.get_logs function
            inputs=[self.log], # takes the previous log content as input to avoid unnecessary updates
            outputs=[self.log], # retrieves updated log content for live UI feed
            show_progress="hidden",
            queue=False,
        )
